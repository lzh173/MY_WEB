...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .points-positive { color: #27ae60; font-weight: bold; }
        .points-negative { color: #e74c3c; font-weight: bold; }
        .notes {
            text-align: left;
            max-width: 300px;
            font-size: 0.9em;
...
<body>
    <div class="container">
        ...
        <div class="controls">
            <a href="?date='. $date .'&raw=true" class="btn" download="'. $date .'.json">
                <i class="fas fa-download"></i> 下载原始数据
            </a>
        ...
        <div class="timestamp">
            <i class="fas fa-clock"></i> 上次提交时间: '. $timestamp .'
        </div>
        <h2 style="text-align: center; margin: 40px 0 20px 0; color: #2c3e50;">
            <i class="fas fa-user-graduate"></i> 全班学号排序
        </h2>
        <table>
            <thead>
                <tr>
...
function handlePostRequest() {
    ...
    if (empty($json)) sendJsonResponse(false, '没有接收到数据');

    $data = json_decode($json, true);
    if ($data === null) sendJsonResponse(false, '无效的JSON数据: ' . json_last_error_msg());
    ...
    if ($result === false) sendJsonResponse(false, '数据保存失败，请检查目录权限');
...
function handleGetRequest() {
    ...
    $date = isset($_GET['date']) ? $_GET['date'] : date('Y-m-d');
    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $date)) sendJsonResponse(false, '日期格式不正确');

    $filename = $dataDir . $date . '.json';
    if (!file_exists($filename)) sendJsonResponse(false, '未找到' . $date . '的数据');

    $data = file_get_contents($filename);
    if ($data === false) sendJsonResponse(false, '无法读取数据文件');
